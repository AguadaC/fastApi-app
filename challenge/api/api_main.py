...
async def load_complete_record(lead: AddLeadRecord, request: Request):
    ...
    2. If the student does not exist, creates a new student record with the provided information.
    ...
    Args:
        lead (AddLeadRecord): The lead record containing student information, including DNI, name, email, phone, 
                              address, subject, career, enrollment year, and time taken.
        request (Request): The FastAPI request object, used for logging and handling the request context.

    Returns:
        ResponseSubjectEnroll: A response containing the enrollment ID of the student in the subject.
    """
    ...
    try:
        ...
        student_id = await db_handler.create_student(dni=lead.dni,
                                                     name=lead.name,
                                                     email=lead.email,
                                                     phone=lead.phone,
    ...
    try:
        ...
        await db_handler.enroll_student_in_a_career(student_id=student_id,
                                                    career_id=career_id,
    ...
    try:
        enroll_id = await db_handler._get_subject_enrollment_id(student_id=student_id,
                                                                career_subject_id=career_subject_id,
                                                                enroll_times=lead.enroll_times)
    except UnenrolledStudent:
        enroll_id = await db_handler.enroll_student_in_a_subject(student_id=student_id,
                                                                 career_subject_id=career_subject_id,
                                                                 enroll_times=lead.enroll_times)
...
async def get_record_by_id(request: Request, record_id: int = Path(gt = 0)):
    ...
    Args:
        ...
        record_id (int): The ID of the lead record to retrieve. Must be greater than 0.

    Returns:
        RetriveLeadRecord: A model containing the details of the lead, 
                           including student information, subject, 
                           career, and enrollment year.
    Raise:
...
async def get_all_records(request: Request, start: int = Query(0, ge=0), limit: int = Query(10, gt=0)):
    """
    Retrieve all complete records with pagination.

    Args:
        request (Request): The FastAPI request object, used for logging.
        start (int): The index to start fetching records from. Must be >= 0. Default is 0.
        limit (int): The maximum number of records to return. Must be > 0. Default is 10.

    Returns:
        List[RetriveLeadRecord]: A list of lead records starting from 'start' index up to 'limit'.
    """
    logger = request.app.logger
