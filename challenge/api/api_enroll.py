...
async def enroll_student_in_a_career(request: Request, student_and_career: EnrollStudentToCareer):
    ...
    This endpoint allows the enrollment of a student in a career based on the student's DNI and the career name provided.

    Args:
        request (Request): The FastAPI request object, used for logging.
        student_and_career (EnrollStudentToCareer): The data model containing the student's DNI,
            career name, and enrollment year.

    Raises:
        StudentCareerEnroll: If the student is already enrolled in the specified career.

    Returns:
        ResponseStudentCareer: An object containing the ID of the newly created student-career enrollment record.
    """
...
async def enroll_student_in_a_subject(request: Request, student_career_subject: EnrollStudentToSubject):
    ...
    This endpoint allows the enrollment of a student in a specific subject based on the student's DNI,
    the career name, and the subject name provided.

    Args:
        request (Request): The FastAPI request object, used for logging.
        student_career_subject (EnrollStudentToSubject): The data model containing the student's DNI,
            career name, subject name, and enrollment times.

    Returns:
        ResponseSubjectEnroll: An object containing the ID of the newly created student-subject enrollment record.
    """
    logger = request.app.logger
